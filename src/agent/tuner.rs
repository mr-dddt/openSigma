...
impl SignalTuner {
    ...
    pub fn should_tune(&self) -> Option<TuneTrigger> {
        // Trigger 1: every N trades
        ...
        // Trigger 2: 10min inactivity (no signal passed filters)
    ...
    pub fn apply_tune(&mut self, config: &mut Config, decision: &TuneDecision) {
        for adj in &decision.adjustments {
            ...
            match adj.param.as_str() {
                "ema_cross_weight" => {
                    config.signals.ema_cross_weight = clamped as i32;
                }
                "cvd_weight" => {
                    config.signals.cvd_weight = clamped as i32;
                }
                "rsi_weight" => {
                    config.signals.rsi_weight = clamped as i32;
                }
                "ob_weight" => {
                    config.signals.ob_weight = clamped as i32;
                }
                "stoch_rsi_weight" => {
                    config.signals.stoch_rsi_weight = clamped as i32;
                }
                "strong_threshold" => {
                    config.signals.strong_threshold = clamped as i32;
                }
                "lean_threshold" => {
                    config.signals.lean_threshold = clamped as i32;
                }
                "rsi_oversold" => {
                    config.signals.rsi_oversold = clamped;
                }
                "rsi_overbought" => {
                    config.signals.rsi_overbought = clamped;
                }
                "min_atr_pct" => {
...
/// Weights: max ±1 per adjustment. Thresholds: max ±2. RSI thresholds: max ±10. ATR: max ±0.02.
